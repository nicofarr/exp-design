...
\begin{frame}
  \frametitle{Positionnement du cours}
  Séances : 
  \begin{enumerate}
  \item 16/01 matin : Neurosciences et neuromythes (D. Toquet).\\ après-midi : Pratique Musicale (C. Rocher)
  \item \alert{23/01 matin : Introduction to Experimental Design (N. Farrugia).} \\ après-midi : Pratique Musicale (C. Rocher)
...
\begin{frame}
  \frametitle{Notions clés}
  \begin{block}{Variables}
    \begin{itemize}
      \item Variable \textit{indépendante}
      \item Variable \textit{dépendante}
      \item \textit{Covariée}
    \end{itemize}
  \end{block}
  \uncover<2>{
