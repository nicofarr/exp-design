...
\begin{columns}
 ...
 Quelques caractéristiques clés de l'EEG
 	\begin{itemize}
 		\item Tension (V) entre une électrode sur le cuir chevelu et une référence
...
\begin{frame}{Hypothèse}
  ...
  \uncover<2>{Exigences d'une hypothèse :
  \begin{itemize}
    \item \textbf{Testabilité} : on doit pouvoir en manipuler les facteurs et en mesurer l'effet
    \item \textbf{Falsifiabilité} : il doit être possible de la réfuter avec des données
    \item \textbf{Précision} : on doit pouvoir en définir les termes et conditions de manière spécifique (opéralionalisée).
  \end{itemize}}
...
\begin{frame}
  ...
  \begin{block}{Groupes}
    \begin{itemize}
      \item Groupe expérimental : sujet à la manipulation expérimentale (variable indépendante)
      \item Groupe contrôle : pas de manipulation expérimentale (e.g. influence des covariés)
    \end{itemize}
...
\begin{frame}
  ...
  \begin{block}{Hypothèse nulle et hypothèse alternative}
    \begin{itemize}
      \item \textit{Hypothèse nulle $H_0$} = pas de différence entre les conditions expérimentales / groupes.
      \item \textit{Hypothèse alternative $H_1$} = différence.
    \end{itemize}
  ...
  \begin{block}{Tests d'hypothèse}
    Utilisation de \textbf{Tests statistiques} sur la variable dépendante pour tester / rejeter $H_0$ (soutenir $H_1$).
  \end{block}
  }
